...
- Eleva.care is a healthcare technology marketplace. We connect clients with independent, licensed practitioners. We are not a healthcare provider.
- We act as:
  - Data Controller for platform operations (accounts, bookings, payments, support, analytics)
  - Data Processor for clinical notes and files created by practitioners (they are the Controller; we provide secure storage)
...
- Clinical data (when used): practitioner‑created notes/files stored encrypted (practitioner controls content)
...
Legal bases include contract performance, legitimate interests (security, product improvement), consent (where required), and legal compliance. For clinical data, practitioners rely on healthcare bases (e.g., GDPR Art. 9(2)(h)); storage may also rely on your explicit consent.
...
- Clinical data: kept per practitioner instructions and applicable healthcare laws
