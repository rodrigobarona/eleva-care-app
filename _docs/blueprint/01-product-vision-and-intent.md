...
> **Persona ↔ role mapping**: in product copy and UI, this user is always called a _Patient_. Their **WorkOS role slug**, however, is the default `member` — there is no custom `patient` or `user` role in v2. (See [18-rbac-and-permissions.md](18-rbac-and-permissions.md) for the full taxonomy.)
