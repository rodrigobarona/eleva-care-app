...
/**
 * Expert Resources Layout
 *
 * Sidebar layout for expert documentation matching Eleva Care brand styling.
...
export default async function ExpertDocsLayout({
  children,
}: LayoutProps) {
  const locale = await getLocale();
  const tree = expertSource.pageTree[locale] ?? expertSource.pageTree.en;
...
function NavigationItem({
  ...
  if (item.type === 'page' && item.url) {
    ...
    // e.g., /docs/expert/en/getting-started -> /docs/expert/getting-started
    const urlWithoutLocale = item.url.replace(`/docs/expert/${locale}`, '/docs/expert');
    return (
      <Link
